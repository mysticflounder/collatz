...
def draw_chord_frame(ax, k, show_tree_edges=True):
    ...
    # Tree edges (faint straight chords) for small k only
    if show_tree_edges and k <= 8:
        segs = []
        for i in range(n):
            j = succ[i]
            if i not in cycle_membership:
                segs.append([(node_x[i], node_y[i]),
                             (node_x[j], node_y[j])])
        if segs:
            lc = LineCollection(segs, colors='#222244',
                                linewidths=0.3, alpha=0.12)
            ax.add_collection(lc)

    # Node dots
    dot_size = max(0.3, 60.0 / n)
    ax.scatter(node_x, node_y, s=dot_size,
               c='#333366', alpha=0.25, zorder=1)
    ...
    for ci, cycle in enumerate(cycles):
        ...
        if cycle_len == 1:
            ...
            dx = node_x[node] * 0.08
            dy = node_y[node] * 0.08
            circ = Circle((node_x[node] + dx, node_y[node] + dy), 0.06,
                          fill=False, edgecolor=color, linewidth=2.5,
                          zorder=4)
            ax.add_patch(circ)
            ax.scatter([node_x[node]], [node_y[node]],
                       s=40, c=color, zorder=5,
            ...
            ms = max(3, 10 - k // 2)
            ax.scatter(cx, cy, s=ms ** 2, c=color, zorder=5,
    ...
    # Title
    if num_ghost > 0:
        ax.set_title('GHOST CYCLE DETECTED', color='#00e5ff',
                     fontsize=16, fontweight='bold', pad=12)
    else:
        ax.set_title('Syracuse Map \u2014 Functional Graph',
                     color='#888899', fontsize=14, pad=12)


def animate_functional_graph(k_min=3, k_max=13,
                             output='analysis/functional_graph.gif'):
    ...
    try:
        ...
        for k in k_values:
            draw_chord_frame(ax, k, show_tree_edges=(k <= 8))
            fig.savefig(f'analysis/chord_k{k:02d}.png',
                        dpi=120, facecolor=fig.get_facecolor())
            print(f'  Saved analysis/chord_k{k:02d}.png')
    plt.close(fig)


def plot_digit_stabilization(output='analysis/digit_stabilization.png'):
    ...
    r_val = sum(3 ** (total_l - 1 - i) * (2 ** partial_sums[i])
    ...
    ax.imshow(grid, aspect='auto', origin='lower', cmap=cmap,
              extent=[min(k_range) - 0.5, max(k_range) - 0.5,
                      -0.5, max_bits - 0.5])

    # Mark k=12 (first materialization)
    ax.axvline(x=12, color='#ffd700', linestyle='--', linewidth=1.5,
               alpha=0.7)
    ...
    for ki, k in enumerate(k_range):
        ax.plot([k - 0.5, k + 0.5], [k - 0.5, k - 0.5],
                color='#ff5722', linewidth=1, alpha=0.5)

    ax.set_xlabel('Resolution k', color='white', fontsize=12)
    ax.set_ylabel('Bit position', color='white', fontsize=12)
    ax.set_title('2-Adic Digit Stabilization: D = -601 Ghost',
...
def plot_cycle_gallery(k_min=3, k_max=13,
                       output='analysis/cycle_gallery.png'):
    ...
    fig.suptitle('Syracuse Functional Graph by Resolution',
                 color='white', fontsize=18, y=1.01)
    fig.tight_layout()
...
def plot_ghost_contrast(output='analysis/ghost_contrast.png'):
    ...
    for ax, k in zip(axes, key_k):
        draw_chord_frame(ax, k, show_tree_edges=False)
